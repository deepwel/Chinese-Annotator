...
class EmbeddingExtractor(object):
    """
    主要用EmbeddingExtractor, 含char2vec/sen2vec等
    目前可以实现了char2vec extractor/sen2vec extractor
    """
    ...
    def char_process(self, token, **kwargs):
        """
        Args:
            token: char
        ...
        if tokens is not None:
            for token in tokens:
                ...
                if token in self.embedding:
                    embeddings.append(self.char_process(token))
        if len(embeddings) > 0:
